...
The first paragraph of RFC 3170 states:
...
not configured properly.  Initially we ran on Virtual Wall 2, as this is
offered by default but it turns out to have the issues identified below;
after investigation, we moved all experiments to Virtual Wall 1, which
had fewer issues.
...
This issue appeared in every single experiment we booted on
virtual wall 2, until we decided to run everything on virtual
wall 1 only, even though that limited the size of experiment
we could run.
...
configured. We informed support of these issues, but we do not have
any information about when they will be investigated.
