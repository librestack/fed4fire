...
more with the corresponding result in terms of speed.

Note that servers in the multicast experiment don't just send data
all the time, they use MLDv2 (Multicast Listener Discovery version
2)~\cite{rfc:3810} and they only start sending when the first client
starts up successfully.  This introduces a slight delay but it will
be an important point for future experiments.
...
this extra delay: MLDv2, mentioned in the single-LAN results, works
on a LAN, so the routers need to forward this information before the
servers know they need to start sending; and once the multicast data
starts flowing, the kernel will need to query a routing daemon to know
what to do with it.  However we can still see that multicast remains
unaffected by scheduling, retaining the advantage already identified
in the single-LAN case.  And we still expect larger experiments to show
this difference between unicast and multicast more clearly.
