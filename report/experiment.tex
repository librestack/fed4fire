...
designed to provide experimental evidence of this efficiency by comparing
three methods of sending the same data to a large number of nodes, as
...
Section~\ref{network} describes the simulated network technology and
the parameters we vary.

Section~\ref{experiment} describes the experiments we run on each
set of parameter values.

Section~\ref{results} analyses the result of the experiment and compares
the efficiency of unicast and multicast.
...
clients may be more or less up-to-date so they each one may request a
...
one; a future experiment may consider the more complex ``real life''
scenario.
