...
\label{configuration}
Several RFCs, for example RFC 2710~\cite{rfc:2710}, require that routers
on an IPv6 network perform the router part of MLD, i.e.\ they forward
multicast packets to all nodes which listen for them, and to no other
nodes.  This experiment requires this to produce the most useful data,
or in some cases, to produce any data at all.

While running the experiment we have determined that some testbeds were
not configured as required and either forwarded all traffic independently
of the presence of listeners, or worse did not forward traffic at all,
even to nodes requesting it. It is essential to verify the degree of
compliance with RFC TODO before repeating the experiment, as this will
affect the results.
...
run the experiment, as well as being the one fully compliant with
RFC TODO.

If the packets arrive at all nodes, the experiment can still proceed,
but the network usage results for the multicast experiment will not
be accurate.  It means that the switch has not been configured properly,
and if possible the experiment should be moved to a different testbed.

In summary, we need to warn anybody wishing to repeat this experiment
to first make sure that the system they are using is configured in
accordance with RFC TODO, to avoid wasting their time on an experiment
which will produce no results.
